...
router.post("/", async (req, res) => {
    ...
    if (!id || !quantity) {
        res.setHeader('Content-Type', 'application/json');
        return res.status(400).json({ error: `Faltan datos: id, product son necesarios` })
    }
    ...
    try {
        let newCart = await cm.createCart(product = { code, price, quantity })
        res.setHeader('Content-Type', 'application/json');
...
router.put("/:cid", async (req, res) => {
    ...
    try {
        let resultado = await cm.updateCart(cid, {code, price, quantity})
